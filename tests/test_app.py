...
from claudechic.formatting import CONTEXT_USAGE_HEADING
...
async def test_context_command_assistant_message_routes_to_command_output():
    ...
    msg = AssistantMessage(
        content=[TextBlock(text=f"{CONTEXT_USAGE_HEADING}\n\n**Tokens:** 19k / 1m\n")],
    ...
    assert CONTEXT_USAGE_HEADING in payload
...
async def test_streamed_textblock_does_not_duplicate_command_output():
    ...
    agent._current_text_buffer = f"{CONTEXT_USAGE_HEADING} already streamed"

    msg = AssistantMessage(
        content=[TextBlock(text=f"{CONTEXT_USAGE_HEADING} already streamed")],
        model="claude-opus-4-7",
    )
    await agent._handle_sdk_message(msg, {})
